  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-b from-cyan-100 to-white py-12 px-4">
        ...
        <motion.button
          onClick={toggleCart}
          className={`flex items-center bg-cyan-500 hover:bg-cyan-600 text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition duration-300 ease-in-out ${isShaking ? "animate-shake" : ""}`}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          <svg
        ...
        {/* Cart Modal */}
        {isCartOpen && (
          <div className="absolute top-12 right-0 bg-white shadow-lg rounded-lg w-96 max-h-[70vh] z-50 overflow-y-auto">
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
            {filteredMenu.map((item) => (
              <div key={item.name} className="bg-white rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-xl p-6 flex flex-col justify-between">
                <div className="flex flex-col items-center">
                  ...
                  />
                  <h3 className="text-xl font-semibold text-cyan-600 text-center mb-2">{item.name}</h3>
                  <p className="text-gray-600 text-center mb-2">{item.description}</p>
                  <p className="text-2xl font-bold text-cyan-600 text-center mb-4">₹{item.cost.toFixed(2)}</p>
                </div>
                <button
